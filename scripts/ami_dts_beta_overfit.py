"""overfit 2-fold: 적응 β(2.0,1.0) vs δ_eff, 각 데이터 2-fold(짝/홀) oracle. 양쪽 다 strict면 robust."""
...
def beta_sig(e,gold,A=2.0,B=1.0,L0=8,g_rho=0.15,rho_min=0.05,lam=0.6):
...
DSETS={ds:(*load(ds,"test"),False) for ds in ["tiage","dialseg711","superseg"]}
...
DSETS["AMI"]=(aem,ag,an,True)
print("overfit 2-fold: 적응β(2.0,1.0) vs δ_eff oracle, 짝/홀 fold 각각. (양쪽 다 adaptive>δ_eff면 robust)")
print(f"{'dataset':<12}{'fold':<6}{'δ_eff':>8}{'adaptive':>10}{'   판정':>8}")
allok=True
for ds,(em,go,ns,tol) in DSETS.items():
    for nm,idx in [("even",range(0,len(em),2)),("odd",range(1,len(em),2))]:
        idx=list(idx)
        de=np.mean([oc_one(np.asarray(delta_eff_seq(em[i])),go[i],ns[i],tol) for i in idx])
        ad=np.mean([oc_one(beta_sig(em[i],go[i]),go[i],ns[i],tol) for i in idx])
        ok=ad>de; allok=allok and ok
        print(f"{ds:<12}{nm:<6}{de:>8.3f}{ad:>10.3f}{('  ✓' if ok else '  ✗FAIL'):>8}", flush=True)
print(f"\n전체 fold 모두 adaptive>δ_eff : {'✓ robust (overfit 아님)' if allok else '✗ 일부 fold 회귀'}")
