#!/usr/bin/env python3
"""Hi-OnTop p60/p70/p80 for ibm-granite/granite-embedding-97m-multilingual-r2.

harness = run_encoder_comparison.py 함수 import.
embedding 은 신규 인코더라 캐시 없음 → 첫 실행 시 인코딩 후 pkl 저장.
결과 → outputs/experiments/2026-06-03_granite_percentile/REPORT.md
"""
...
EXP_NAME = "2026-06-03_granite_percentile"
...
def main() -> None:
    ...
    # ---- REPORT.md ----
    lines = [
        f"# Hi-OnTop Granite 인코더 비교 ({EXP_NAME})",
        "",
        "## 실험 setup",
        f"- 인코더: `{GRANITE_MODEL}` (dim=384, multilingual, ONNX quint8_avx2)",
