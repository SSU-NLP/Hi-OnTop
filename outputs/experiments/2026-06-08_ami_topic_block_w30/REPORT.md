# AMI 화제분절 — 60초 블록 단위 Hi-OnTop (2026-06-08_ami_topic_block_w30)

## 실험 setup
- **목적**: turn 단위로 붕괴한 Hi-OnTop(AUC 0.567)을 **30초 시간 블록** 단위로 재구성하면 화제 cohesion·경계 신호가 회복되는지 실측.
...
- **블록 구성**: 각 발화의 begin_time 기준 30초 고정 bin 에 텍스트 이어붙임 → 한 블록 = 한 덩어리 텍스트. 빈 블록 제외.
...
- **블록 통계**: 평균 54.2 블록/미팅, 경계 밀도 11.2%.
- **AUC (threshold-free 분리력)**: **0.669** (turn 단위 0.567 대비).

## 결과 (test split, percentile sweep)

| p | δ\* | Pk ↓ | WD ↓ | F1 ↑ | Score ↑ |
|--:|--:|--:|--:|--:|--:|
| 50 | 0.632 | 0.597 | 0.798 | 0.177 | 0.240 |
| 60 | 0.675 | 0.579 | 0.731 | 0.189 | 0.267 |
| 70 | 0.722 | 0.546 | 0.675 | 0.185 | 0.287 |
| 80 | 0.787 | 0.459 | 0.545 | 0.208 | 0.353 |
| 90 | 0.850 | 0.333 | 0.412 | 0.197 | 0.412  ← best |

## 해석
- best = **p90** (Score 0.412, F1 0.197, Pk 0.333).
- threshold-free AUC 0.669 — turn 단위(0.567)보다 분리력 상승. 단위(granularity)가 핵심이었음을 직접 확인.
...
## 한계 / 검증 미해결
- **블록 경계 해상도 ±30s**: 경계를 블록 시작 단위로만 찍어 위치 정밀도가 거침.
