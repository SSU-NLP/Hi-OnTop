# AMI 화제분절 — 60초 블록 단위 Hi-OnTop (2026-06-08_ami_topic_block_w45)

## 실험 setup
- **목적**: turn 단위로 붕괴한 Hi-OnTop(AUC 0.567)을 **45초 시간 블록** 단위로 재구성하면 화제 cohesion·경계 신호가 회복되는지 실측.
...
- **블록 구성**: 각 발화의 begin_time 기준 45초 고정 bin 에 텍스트 이어붙임 → 한 블록 = 한 덩어리 텍스트. 빈 블록 제외.
...
- **블록 통계**: 평균 37.8 블록/미팅, 경계 밀도 16.1%.
- **AUC (threshold-free 분리력)**: **0.689** (turn 단위 0.567 대비).

## 결과 (test split, percentile sweep)

| p | δ\* | Pk ↓ | WD ↓ | F1 ↑ | Score ↑ |
|--:|--:|--:|--:|--:|--:|
| 50 | 0.577 | 0.491 | 0.692 | 0.323 | 0.365 |
| 60 | 0.617 | 0.449 | 0.626 | 0.307 | 0.385 |
| 70 | 0.659 | 0.429 | 0.566 | 0.281 | 0.392 |
| 80 | 0.713 | 0.407 | 0.495 | 0.246 | 0.397 |
| 90 | 0.793 | 0.366 | 0.410 | 0.187 | 0.399  ← best |

## 해석
- best = **p90** (Score 0.399, F1 0.187, Pk 0.366).
- threshold-free AUC 0.689 — turn 단위(0.567)보다 분리력 상승. 단위(granularity)가 핵심이었음을 직접 확인.
...
## 한계 / 검증 미해결
- **블록 경계 해상도 ±45s**: 경계를 블록 시작 단위로만 찍어 위치 정밀도가 거침.
