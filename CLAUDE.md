...
2. **내용 = 문제 정의 + 모든 시도(설정·조건) + 결과 (필수 갱신).** 지정된 문제로 작업할 때마다 그 문제의
   handoff 파일에:
   - 시도한 **모든 방법의 자세한 설정/조건**(HP·데이터·method·config·seed·스크립트 경로 등)을 빠짐없이 기록,
   - 각 시도의 **결과**(수치·판정·실패사유)를 업데이트한다.
   선택이 아니라 **필수** — 지정 문제로 실험/시도하면 *반드시* 해당 handoff 를 갱신한다(실험 결과 REPORT
   규칙과 별개로, handoff 는 그 문제의 "모든 시도 ledger" 역할).
